...
public class Sale {
	 ...
	 * the rest.
	 */
	public boolean processLineMessage(String saleNotice) {

		/*
		 * MessageParser helps to parse the incoming messages and obtain product
		 * sale information.
		 */
	...
	private void setProductTotalPrice() {
		...
		if (!product.getAdjustmentOperator().isEmpty()) {
			adjustedPrice = priceAdjuster.calculateAdjustedPrice();
			log.setAdjustmentReports(priceAdjuster.adjustmentReport());
